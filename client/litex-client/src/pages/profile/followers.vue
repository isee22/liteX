<template>
  <view class="twitter-layout">
    ...
    <view class="main-content">
      ...
      <view class="tabs">
        <view class="tab" @click="goFollowing">正在关注</view>
        <view class="tab active">关注者</view>
      </view>
      <view class="user-list">
        <view v-for="item in followers" :key="item.id" class="user-item" @click="goProfile(item.user?.id)">
          ...
          <button v-if="!isMe(item.user?.id)" class="follow-btn" :class="{ following: item.user?.isfollowing }" @click.stop="toggleFollow(item.user)">
            {{ item.user?.isfollowing ? '已关注' : '关注' }}
          </button>
        </view>
        <view v-if="!followers.length" class="empty">暂无关注者</view>
      </view>
    </view>
    <RightSidebar />
  </view>
...
const fetchFollowers = async () => {
  const id = userId.value || 'me'
  const res = await get(`/user/${id}/followers`)
  followers.value = res.data?.list || res.data || []
}
...
const goFollowing = () => uni.redirectTo({ url: `/pages/profile/following?id=${userId.value}` })
...
.follow-btn { background: var(--btn-primary-bg); color: var(--btn-primary-text); border: none; border-radius: 9999px; padding: 8px 16px; font-size: 14px; font-weight: bold; }
.follow-btn.following { background: transparent; border: 1px solid var(--text-secondary); color: var(--text-primary); }
